...
\item
  Design a study and collect data. \emph{Modules 1--2}
\item
  Summarize and visualize the data. \emph{Modules 3--4}
\item
  Use statistical analysis methods to draw inferences from the data. \emph{Modules 5 - 11}
\item
...
Reading guides are designed to be completed while reading the required sections in the course textbook (\url{https://mtstateintrostats.github.io/IntroStatTextbook/index.html}) to aid students in taking notes. These reading guides are not turned in in class but will be useful in understanding key concepts each week. Solutions to the reading guides will be posted on D2L.

\subsection{Module 1 Reading Guide: Basics of Data}\label{module-1-reading-guide-basics-of-data}
...
\section{Video Notes: Intro to data}\label{video-notes-intro-to-data}
...
\section{Guided Activity 1: Intro to Data Analysis}\label{guided-activity-1-intro-to-data-analysis}
...
\tightlist
\item
  Brainstorm with your table on ways to summarize the variable \texttt{Residency} both visually and mathematically. Write down your tables ideas.
\end{enumerate}
...
\tightlist
\item
  Your instructor will use RStudio (a statistical software package) to create a graph and numerical summaries of the variable \texttt{Residency}. Draw the graph created here.
\end{enumerate}
...
\tightlist
\item
  Your instructor will use RStudio to find the count for each level of the variable \texttt{Residency}. Calculate the proportion of Stat 216 students that have in-state residency.
\end{enumerate}
...
\tightlist
\item
  Brainstorm with your table on ways to summarize the variable \texttt{Grip\_Strength\_Dominant} both visually and mathematically. Write down your table's ideas.
\end{enumerate}
...
\tightlist
\item
  Your instructor will use RStudio to create a graph and numerical summaries of the variable \texttt{Grip\_Strength\_Dominant}. Draw the graph created here.
\end{enumerate}
...
\section{Reading Guide: Sampling, Study Design, and Scope of Inference}\label{reading-guide-sampling-study-design-and-scope-of-inference}
...
\section{Video Notes: Study Design}\label{video-notes-study-design}
...
\section{Guided Activity 2: American Indian Address}\label{guided-activity-2-american-indian-address}
...
\item
  ...
  \vspace{0.5in}
\item
  Based on your answers to questions 4 and 5, would you say the sampling method used by the class is biased or unbiased? Justify your answer.\\
  \vspace{0.5in}
\item
...
\section{Guided Activity 3: American Indian Address (continued)}\label{guided-activity-3-american-indian-address-continued}
...
Today we will return to the American Indian Address introduced in the guided activity. Suppose instead of attempting to select a representative sample by eye (which did not work), each student used a random number generator to select a simple random sample of 10 words. A \textbf{simple random sample} relies on a random mechanism to choose a sample, without replacement, from the population, such that every sample of size 10 is equally likely to be chosen.
...
\item
  ...
  \vspace{0.3in}
\item
  How does the plot given in this activity compare to the plot generated in the guided activity?
\end{enumerate}
...
\setcounter{enumi}{5}
\item
  The plot labeled ``Statistics'' displays the 1,000 randomly generated sample mean word lengths (the sampling distribution). Sketch this plot below. Include a descriptive \(x\)-axis label and be sure to write down the provided mean and SD (standard deviation) of the distribution.
  \vspace{2in}
\item
  What is the center value (mean) of the distribution created in question 6?
  \vspace{0.3in}
\item
...
\setcounter{enumi}{12}
\item
  Compare the values of the standard deviation of the plots in question 11 and in question 6. Which plot shows the smallest standard deviation?
  \vspace{0.4in}
\item
...
\section{Activity 2: Study Design}\label{activity-2-study-design}
...
\section{Reading Guide: Introduction to R, Categorical Variables, and a Single Quantitative Variable}\label{reading-guide-introduction-to-r-categorical-variables-and-a-single-quantitative-variable}
...
\section{Video Notes: Exploratory Data Analysis of Categorical and Quantitative Variables}\label{video-notes-exploratory-data-analysis-of-categorical-and-quantitative-variables}
...
\tightlist
\item
  What summary statistic is calculated for a two categorical variables?
\end{enumerate}
...
\tightlist
\item
  What four characteristics should be used to compare side-by-side boxplots?
\end{enumerate}
...
\section{Guided Activity 4: Summarizing Categorical Data}\label{guided-activity-4-summarizing-categorical-data}
...
\subsubsection*{Identifing Variables in a data set}\label{identifing-variables-in-a-data-set}
\addcontentsline{toc}{subsubsection}{Identifing Variables in a data set}
...
\tightlist
\item
  Write down the number of observational units and the number of variables collected in the data set.
\end{enumerate}
...
\tightlist
\item
  Write down the number of 4-year institutions in the data set.
\end{enumerate}

\vspace{0.1in}

\subsubsection*{Summarizing categorical variables}\label{summarizing-categorical-variables}
\addcontentsline{toc}{subsubsection}{Summarizing categorical variables}

An important part of understanding data is to create visual pictures of what the data represent. In this activity, we will create graphical representations of categorical data.

There are four categorical variables in this data set. \texttt{Sector}, \texttt{Size}, \texttt{LandGrant}, and \texttt{Tenure}.
...
If we wanted to know how many 4-year institutions in our data set were in each category of size, we could create a frequency bar plot of the variable \texttt{Size}.

\begin{itemize}
\item
  In the R code below (and the provided R script file), we will enter the variable name, \texttt{Size} (\emph{note the capital S}), for \texttt{variable} into the \texttt{ggplot} code. This is in line 18 in the R script file.
\item
  Highlight and run lines 16--22 to create the plot. Note: this is a \textbf{frequency} bar plot plotting counts (the number of institutions within each level of \texttt{Size} is displayed on the \(y\)-axis).
\end{itemize}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{IPEDS}\SpecialCharTok{$}\NormalTok{Size }\OtherTok{\textless{}{-}} \FunctionTok{factor}\NormalTok{(IPEDS}\SpecialCharTok{$}\NormalTok{Size, }\AttributeTok{levels =} \FunctionTok{c}\NormalTok{(}\StringTok{"\textless{} 1000"}\NormalTok{, }\StringTok{"1,000{-}4,999"}\NormalTok{,}
                                            \StringTok{"5,000{-}9,999"}\NormalTok{, }\StringTok{"10,000{-}19,999"}\NormalTok{, }\StringTok{"20,000 and above"}\NormalTok{))}
\NormalTok{IPEDS }\SpecialCharTok{\%\textgreater{}\%} \CommentTok{\# Data set piped into...}
\FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Size)) }\SpecialCharTok{+}   \CommentTok{\# This specifies the variable}
  ...
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{title =} \StringTok{"Frequency Bar Plot of Size of 4{-}year US Institutions"}\NormalTok{,  }
       \CommentTok{\# Give your plot a title}
       \AttributeTok{x =} \StringTok{"Student Population Size"}\NormalTok{,   }\CommentTok{\# Label the x axis}
...
\tightlist
\item
  Using the bar chart created, estimate how many institutions have more than 10,000 students.
\end{enumerate}

\vspace{0.2in}
\newpage

We could also choose to display the data as a proportion in a \textbf{relative frequency} bar plot. To find the relative frequency, the count in each level of \texttt{Size} is divided by the sample size. This calculation is the sample proportion for each level of size. Notice that in this code we told R to create a bar plot with proportions.

\begin{itemize}
\item
  In the R script file, enter the variable name \texttt{Size} for \texttt{variable} in line 27.
\item
...
\FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Size)) }\SpecialCharTok{+}   \CommentTok{\# This specifies the variable}
  ...
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{title =} \StringTok{"Relative Frequency Bar Plot of Size for 4{-}year }
\StringTok{       US Institutions"}\NormalTok{,  }
       \CommentTok{\# Give your plot a title}
       \AttributeTok{x =} \StringTok{"Student Population Size"}\NormalTok{,   }\CommentTok{\# Label the x axis}
...
In R, to get the counts for each level of a variable use the function \texttt{count}.

\begin{itemize}
\item
  In the R script file, enter the variable name \texttt{Size} for \texttt{variable} in line 35.
\item
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{IPEDS }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{count}\NormalTok{(Size)}
\end{Highlighting}
\end{Shaded}
...
\tightlist
\item
  Calculate the proportion of 4-year Institutions that have a student population size of between 1,000-4,999? Use appropriate notation.
\end{enumerate}
...
Is there an association between the student population size of an institution and whether it is public or private institution? In this instance we will treat the variable \texttt{Sector} as the explanatory variable and \texttt{Size} as the response variable.

\begin{itemize}
\item
  Fill in the name of the explanatory variable, \texttt{Sector} for explanatory and name of the response variable, \texttt{Size} in line 39 in the R script file.
\item
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{IPEDS }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{group\_by}\NormalTok{(Sector) }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{count}\NormalTok{(Size) }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{print}\NormalTok{(}\AttributeTok{n=}\DecValTok{10}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\setcounter{enumi}{6}
\item
  Calculate the proportion of institutions that are public. Use appropriate notation.
  \vspace{0.4in}
\item
  Calculate the proportion of public institutions that have 1000 - 4999 students. Use appropriate notation.
  \vspace{0.4in}
\item
  Calculate the proportion of private institutions that have 1000 - 4999 students. Use appropriate notation.
  \vspace{0.4in}
\item
  Calculate the difference in proportion of institutions that have 1000 - 4999 students for those that are public minus those that are private. Give the appropriate notation. Use public minus private as the order of subtraction.
  \vspace{0.5in}
\item
  Interpret the difference in proportion calculated in question 9 in context of the study.
\end{enumerate}

\vspace{0.6in}

Two types of plots can be created to display two categorical variables. To examine the association between the student population size of an institution and whether it is public or private institution, we will first create a segmented bar plot of \texttt{Sector} segmented by \texttt{Size}.

\begin{itemize}
\item
  To create the segmented bar plot enter the variable name, \texttt{Sector} for \texttt{explanatory} and the variable name, \texttt{Size} for \texttt{response} in line 44 in the R script file.
\item
...
\begin{Highlighting}[]
\NormalTok{IPEDS }\SpecialCharTok{\%\textgreater{}\%} \CommentTok{\# Data set piped into...}
\FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Sector, }\AttributeTok{fill =}\NormalTok{ Size)) }\SpecialCharTok{+}   \CommentTok{\# This specifies the variables}
  ...
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{title =} \StringTok{"Segmented Bar Plot of Student Population Size by }
\StringTok{       Sector for 4{-}year Institutions"}\NormalTok{,  }\CommentTok{\# Make sure to title your plot }
...
\tightlist
\item
  From the segmented bar plot, which sector has the highest proportion of student population size of 1000 - 4999?
\end{enumerate}
...
\NormalTok{IPEDS }\SpecialCharTok{\%\textgreater{}\%} \CommentTok{\#Data set piped into...}
  \FunctionTok{ggplot}\NormalTok{() }\SpecialCharTok{+}   \CommentTok{\#This specifies the variables}
  \FunctionTok{geom\_mosaic}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x=}\FunctionTok{product}\NormalTok{(Sector), }\AttributeTok{fill =}\NormalTok{ Size)) }\SpecialCharTok{+}  \CommentTok{\#Tell it to make a mosaic plot}
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{title =} \StringTok{"Mosaic Plot of Student Population Size by }
\StringTok{       Sector for 4{-}year Institutions"}\NormalTok{, }\CommentTok{\#Make sure to title your plot }
       \AttributeTok{x =} \StringTok{"Sector"}\NormalTok{,   }\CommentTok{\#Label the x axis}
       \AttributeTok{y =} \StringTok{""}\NormalTok{) }\SpecialCharTok{+} \CommentTok{\#Remove y axis label}
\FunctionTok{scale\_fill\_grey}\NormalTok{()  }\CommentTok{\# Make figure bw}
\end{Highlighting}
\end{Shaded}
...
\section{Guided Activity 5: Summarizing Quantitative Variables}\label{guided-activity-5-summarizing-quantitative-variables}
...
\subsection{IPEDS continued}\label{ipeds-continued}

We will revist the IPEDS data set now analyzing quantitative variables. These data were collected on a subset of institutions that met the following selection criteria (Education Statistics 2018):
...
We will create both a histogram and a boxplot of the variable \texttt{Graduation\_Rate}.

\begin{itemize}
\item
  Enter the name of the variable in both line 19 and line 26 for \texttt{variable} in the R script file.
\item
  Replace the word title for each plot (lines 21 and 28) between the quotations with a descriptive title. \textbf{A title should include: type of plot, variable or variables plotted, and observational units.}
\item
...
\FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Graduation\_Rate)) }\SpecialCharTok{+}   \CommentTok{\# Name variable to plot}
  ...
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{title =} \StringTok{""}\NormalTok{, }\CommentTok{\# Title for plot}
...
\subsubsection*{Summarizing one categorical and one quantitative variable}\label{summarizing-one-categorical-and-one-quantitative-variable}
\addcontentsline{toc}{subsubsection}{Summarizing one categorical and one quantitative variable}

Is there a difference in retention rates for public and private 4-year institutions? In this part of the activity we will compare retention rates for public and private 4-year institutions. Note that this variable (public or private) is labelled \texttt{Sector} in the data set.
...
\tightlist
\item
  Based on the research question, which variable will we treat as the explanatory variable? Response variable?
\end{enumerate}

\vspace{0.3in}

The \texttt{favstats()} function from the \texttt{mosaic} package gives the summary statistics for a quantitative variable. The summary statistics give the two measures of center and two measures of spread for retention rates between sector for 4-year institutions.
...
\begin{Highlighting}[]
\NormalTok{IPEDS }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{reframe}\NormalTok{(}\FunctionTok{favstats}\NormalTok{(Retention}\SpecialCharTok{\textasciitilde{}}\NormalTok{Sector)) }\CommentTok{\#Gives the summary statistics}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\NormalTok{IPEDS }\SpecialCharTok{\%\textgreater{}\%}  \CommentTok{\# Data set piped into...}
  \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{y =}\NormalTok{ Retention, }\AttributeTok{x =}\NormalTok{ Sector))}\SpecialCharTok{+}  \CommentTok{\# Identify variables}
  \FunctionTok{geom\_boxplot}\NormalTok{()}\SpecialCharTok{+}  \CommentTok{\# Create box plot}
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{title =} \StringTok{"Side{-}by{-}side Boxplot of Retention Rate among Public and}
\StringTok{       Private 4{-}year Institutions"}\NormalTok{,  }
       \CommentTok{\# A title should include: type of plot,variable or variables plotted, and observational units}
       \AttributeTok{x =} \StringTok{"Sector"}\NormalTok{,    }\CommentTok{\# x{-}axis label}
...
\tightlist
\item
  Interpret the difference in mean retention rates found in question 5.
\end{enumerate}
...
\section{Module 3 Lab: EDA Review}\label{module-3-lab-eda-review}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{movies }\OtherTok{\textless{}{-}} \FunctionTok{read.csv}\NormalTok{(}\StringTok{"https://www.math.montana.edu/courses/s216/data/Movies2016\_F24.csv"}\NormalTok{)}
...
To show the effect of outliers on the measures of center and spread, the largest values in the data set were reduced by 100 \$MM to decrease the range of the data set. This variable is called \texttt{revenue\_reduced}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{movies }\SpecialCharTok{\%\textgreater{}\%} \CommentTok{\# Data set piped into...}
    \FunctionTok{summarise}\NormalTok{(}\FunctionTok{favstats}\NormalTok{(revenue\_reduced))}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\NormalTok{movies }\SpecialCharTok{\%\textgreater{}\%} \CommentTok{\# Data set piped into...}
\FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ revenue\_reduced)) }\SpecialCharTok{+} \CommentTok{\# Name variable to plot}
\FunctionTok{geom\_boxplot}\NormalTok{() }\SpecialCharTok{+} \CommentTok{\# Create histogram with specified binwidth}
\FunctionTok{labs}\NormalTok{(}\AttributeTok{title =} \StringTok{"Histogram of Decreased Revenue of Movies in 2016"}\NormalTok{, }\CommentTok{\# Title for plot}
\AttributeTok{x =} \StringTok{"Revenue (in Millions)"}\NormalTok{, }\CommentTok{\# Label for x axis}
\AttributeTok{y =} \StringTok{"Frequency"}\NormalTok{) }\CommentTok{\# Label for y axis}
\end{Highlighting}
\end{Shaded}
...
\begin{itemize}
\item
  Enter the variable name \texttt{content\_rating} for explanatory and \texttt{budget} for response in line 33
\item
  Highlight and run lines 31--33 in the provided \texttt{R} script file.
\end{itemize}
...
\NormalTok{movies }\SpecialCharTok{\%\textgreater{}\%} \CommentTok{\# Data set piped into...}
  ...
  \FunctionTok{reframe}\NormalTok{(}\FunctionTok{favstats}\NormalTok{(response}\SpecialCharTok{\textasciitilde{}}\NormalTok{explanatory)) }\CommentTok{\# Find the summary measures for each content rating}
\end{Highlighting}
\end{Shaded}
...
\tightlist
\item
  \textbf{Interpret the difference in means calculated in question 10 in context of the problem.}
\end{enumerate}
...
\tightlist
\item
  \textbf{Report and interpret the standard deviation of the budget amount for movies released in 2016 with a PG rating.}
\end{enumerate}
...
\tightlist
\item
  Calculate the \(IQR = Q_3 - Q_1\) of the budget amount for movies released in 2016 with a PG rating.
\end{enumerate}
...
\begin{itemize}
\item
  Enter the variable \texttt{budget\_mil} for \texttt{response} and the variable \texttt{content\_rating} for explanatory at line 30.
\item
...
\item
  ...
  \vspace{0.2in}
\item
  \textbf{Fifty percent of movies released in 2016 with a PG-13 content rating fall below what value? What is the name of this value?}
  \vspace{0.4in}
\item
...
\begin{itemize}
\item
  Download the provided data set \texttt{ChildrenLightSight.csv} from D2L
\item
  Upload the data set \texttt{ChildrenLightSight} to the RStudio server using the same steps to upload the R script file.
\item
  Enter the name of the data set for \texttt{datasetname} in the R script file in line 48. Don't forget to include .csv (\texttt{ChildrenLightSight.csv}).
\item
  Highlight and run line 48 in the provided \texttt{R} script file to load the data set.
\end{itemize}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Read in data set}
\NormalTok{myopia }\OtherTok{\textless{}{-}} \FunctionTok{read.csv}\NormalTok{(}\StringTok{"datasetname"}\NormalTok{) }
\end{Highlighting}
\end{Shaded}
...
\subsubsection*{Displaying two categorical variables}\label{displaying-two-categorical-variables-1}
\addcontentsline{toc}{subsubsection}{Displaying two categorical variables}
...
\begin{itemize}
\item
  Fill in the name of the explanatory variable, \texttt{Light} for explanatory and name of the response variable, \texttt{Sight} in line 52 in the R script file.
\item
...
\item
  ...
  \vspace{0.3in}
\item
  \textbf{Calculate the difference in proportion of children with no myopia for those that slept with full light minus those who slept with no light. Give the appropriate notation. Use full light minus no light as the order of subtraction.}
  \vspace{0.3in}
\end{enumerate}

\newpage

To examine the differences in level of myopia for the level of light, we will create a segmented bar plot of \texttt{Light} segmented by \texttt{Sight}.

\begin{itemize}
\item
  To create the segmented bar plot enter the variable name, \texttt{Light} for \texttt{explanatory} and the variable name, \texttt{Sight} for \texttt{response} in the R script file in line 57.
\item
...
\FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ explanatory, }\AttributeTok{fill =}\NormalTok{ response)) }\SpecialCharTok{+}   \CommentTok{\# This specifies the variables}
  \FunctionTok{geom\_bar}\NormalTok{(}\AttributeTok{stat =} \StringTok{"count"}\NormalTok{, }\AttributeTok{position =} \StringTok{"fill"}\NormalTok{) }\SpecialCharTok{+}  \CommentTok{\# Tell it to make a stacked bar plot}
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{title =} \StringTok{"Segmented Bar Plot of Night Light Use by Level of Myopia for Children"}\NormalTok{,  }
...
\section{Reading Guide: Two Quantitative Variables and Multivariable Concepts}\label{reading-guide-two-quantitative-variables-and-multivariable-concepts}
...
\section{Guided Activity 6: IPEDS (continued)}\label{guided-activity-6-ipeds-continued}
...
The code below reads in the needed data set, IPEDS\_2018.csv, and filters out the 2-year institutions. Highlight lines 1 -- 11 to load the data set and filter out the 2-year institutions.
...
The create a scatterplot of the 75th percentile Math SAT score by retention rate for 4-year US Higher Education Institutions\ldots{}
...
\section{Activity 3: Moneyball --- Linear Regression}\label{activity-3-moneyball-linear-regression}
...
\begin{itemize}
\item
  Upload the \texttt{Antarctica\_Penguins} csv file
\item
  Upload and open the provided R script file for the module 4 lab.
\item
  Enter the name of the data set (\texttt{Antarctica\_Penguins}) for \texttt{datasetname} in the R script file in line 5. *Note that there are some missing values in the dataset. Line 6 in the R script file removes the NAs.
\end{itemize}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{penguins }\OtherTok{\textless{}{-}} \FunctionTok{read.csv}\NormalTok{(}\StringTok{"datasetname"}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%} \CommentTok{\#Creates the object penguins}
    \FunctionTok{drop\_na}\NormalTok{(body\_mass\_g) }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{drop\_na}\NormalTok{(bill\_depth\_mm) }\CommentTok{\#Removes data points without values}
...
\begin{itemize}
\item
  Enter the variable name \texttt{body\_mass\_g} for \texttt{response} and the variable name \texttt{bill\_depth\_mm} for \texttt{explanatory} in line 30 in the R script file.
\item
...
\FunctionTok{round}\NormalTok{(}\FunctionTok{summary}\NormalTok{(penguinsLM)}\SpecialCharTok{$}\NormalTok{coefficients, }\DecValTok{3}\NormalTok{) }\CommentTok{\# Display coefficient summary}
\end{Highlighting}
\end{Shaded}
...
\subsubsection*{Multivariable plot}\label{multivariable-plot}
\addcontentsline{toc}{subsubsection}{Multivariable plot}
...
\item
  ...
  \rgs
\item
  Does the 95\% confidence interval indicate that the consults has a lower rate of complications than the national average (10\%)? Justify your answer.
  \rgs
\end{enumerate}

\newpage

\section{Video Notes: Inference for One Categorical Variable using Simulation-based Methods}\label{video-notes-inference-for-one-categorical-variable-using-simulation-based-methods}
...
\tightlist
\item
  What was the null value for the study discussed?
\end{enumerate}
...
\section{Guided Activity 7: Sleep Study --- Simulation-based Inference Methods}\label{guided-activity-7-sleep-study-simulation-based-inference-methods}
...
\subsection{Sleep Study}\label{sleep-study}

The Gallup Poll reports that 59\% of Americans get seven or more hours of sleep at night. Recent research (Wahlstrom 2014) on the sleep-wake cycle of teenagers has indicated that biological changes associated with puberty may lead adolescents to require more sleep and feel sleepier at later times. One external factor that can impact their sleep is the early start time of high school. A study was conducted, involving a random sample of 446 students from public schools across three different states, to investigate whether these students sleep for at least 7 hours daily. The results revealed that 296 students sleep for at least 7 hours daily, while 150 sleep for less than 7 hours daily. Is there evidence that the true proportion of public high school students who sleep for at least 7 hours daily is greater than what was reported by the Gallup Poll?
...
\begin{Highlighting}[]
\CommentTok{\# Read in data set}
\NormalTok{sleep }\OtherTok{\textless{}{-}} \FunctionTok{read.csv}\NormalTok{(}\StringTok{"data/LosingSleep.csv"}\NormalTok{)}
\NormalTok{sleep }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{count}\NormalTok{(Outcome)  }\CommentTok{\# Count number in each category}
\end{Highlighting}
\end{Shaded}
...
\tightlist
\item
  Using the \texttt{R} output and the formula given, calculate the summary statistic (sample proportion) to represent the research question. Use appropriate notation.
\end{enumerate}
...
\tightlist
\item
  How far is the summary statistic from the value of 0.59? Is this a meaningful difference?
\end{enumerate}
...
For this study, the parameter of interest is the \textbf{true or population proportion of public high school students who sleep at least 7 hours daily}.
...
\tightlist
\item
  What assumption are we making about the true proportion of public high school students who sleep at least 7 hours daily in the null hypothesis?
\end{enumerate}
...
Remember that when utilizing a hypothesis test, we are evaluating two competing possibilities if the observed statistic is unusual. For this study the \textbf{two possibilities} are either\ldots{}
...
\begin{Highlighting}[]
\FunctionTok{set.seed}\NormalTok{(}\DecValTok{216}\NormalTok{)}
\FunctionTok{one\_proportion\_test}\NormalTok{(}\AttributeTok{probability\_success =} \FloatTok{0.59}\NormalTok{, }\CommentTok{\# Null hypothesis value}
          \AttributeTok{sample\_size =} \DecValTok{446}\NormalTok{, }\CommentTok{\# Enter sample size}
          \AttributeTok{number\_repetitions =} \DecValTok{1000}\NormalTok{, }\CommentTok{\# Enter number of simulations}
          \AttributeTok{as\_extreme\_as =} \FloatTok{0.664}\NormalTok{, }\CommentTok{\# Observed statistic}
          \AttributeTok{direction =} \StringTok{"greater"}\NormalTok{, }\CommentTok{\# Specify direction of alternative hypothesis}
          \AttributeTok{summary\_measure =} \StringTok{"proportion"}\NormalTok{) }\CommentTok{\# Reporting proportion or number of successes?}
\end{Highlighting}
\end{Shaded}
...
\tightlist
\item
  Write down the standard deviation of the null distribution. This value estimates the sample to sample variability in the population of public high school students.
\end{enumerate}
...
\tightlist
\item
  Explain why the null distribution is centered around 0.59.
\end{enumerate}
...
\addcontentsline{toc}{paragraph}{Estimation}

To create an interval estimate of plausible values for the parameter of interest, we will use the \texttt{one\_proportion\_bootstrap\_CI()} function in R (in the \texttt{catstats} package) to simulate the bootstrap distribution of sample proportions and calculate a confidence interval.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{set.seed}\NormalTok{(}\DecValTok{216}\NormalTok{)}
\FunctionTok{one\_proportion\_bootstrap\_CI}\NormalTok{(}\AttributeTok{sample\_size =} \DecValTok{446}\NormalTok{, }\CommentTok{\# Sample size}
                    \AttributeTok{number\_successes =} \DecValTok{296}\NormalTok{, }\CommentTok{\# Observed number of successes}
                    \AttributeTok{number\_repetitions =} \DecValTok{1000}\NormalTok{, }\CommentTok{\# Number of bootstrap samples to use}
                    \AttributeTok{confidence\_level =} \FloatTok{0.95}\NormalTok{) }\CommentTok{\# Confidence level as a decimal}
\end{Highlighting}
\end{Shaded}
...
\tightlist
\item
  Explain why the bootstrap distribution is centered around the value of 0.664.
\end{enumerate}
...
\tightlist
\item
  Is the null value contained within the 95\% confidence interval? Explain how this indicates that there is evidence that the true proportion of public high school students who sleep for at least 7 hours daily is greater than what was reported by the Gallup Poll.
\end{enumerate}
...
\tightlist
\item
  What is the purpose of completing a hypothesis test?
\end{enumerate}
...
\tightlist
\item
  What is the purpose of finding a confidence interval?
\end{enumerate}
...
\def\labelenumi{\arabic{enumi}.}
\item
  A hypothesis test determines the strength of evidence of an effect. The smaller the p-value the more evidence the statistic provides against the null hypothesis.
\item
  A confidence interval gives an interval estimate for the parameter of interest - a range of plausible values.
\item
  The direction of the alternative hypothesis depends on the research question.
\end{enumerate}
...
\subsubsection*{Design a study and collect data}\label{design-a-study-and-collect-data-1}
\addcontentsline{toc}{subsubsection}{Design a study and collect data}
...
\tightlist
\item
  Sketch a \textbf{relative frequency} bar plot of these data.
\end{enumerate}
...
\tightlist
\item
  In what direction (greater or less than) did we count from the statistic to find the number of simulations?
  \vspace{0.3in}
\end{enumerate}
...
\tightlist
\item
  Write out the parameter of interest in words, in context of the study. \emph{Hint: this is the same as in Activity 4 and 5.}
\end{enumerate}
...
\def\labelenumi{\arabic{enumi}.}
\item
  It \_\_\_\_\_\_\_\_\_\_\_\_ (increases/decreases) the chance of a Type 1 Error.
\item
...
\subsection*{Errors, power, and practical importance - Video Chapter12}\label{errors-power-and-practical-importance---video-chapter12}
\addcontentsline{toc}{subsection}{Errors, power, and practical importance - Video Chapter12}
...
\section{Guided Activity 8: Cashless Society}\label{guided-activity-8-cashless-society}
...
\subsection{Cashless Society}\label{cashless-society}

According to published results in 2020, 34\% of U.S. adults reported using only cashless purchases. In a 2022 Pew Research Center study ({``More Americans Are Joining the 'Cashless' Economy''} 2022), a random sample of 6034 U.S. adults was surveyed about their cashless purchase habits. In this 2022 study, 2474 U.S. adults reported using only cashless purchases in a typical week. Is there evidence that the rate of using only cashless purchases among U.S. adults differs from 0.34?

\begin{Shaded}
\begin{Highlighting}[]
 \CommentTok{\# Read in data set}
\NormalTok{cashless }\OtherTok{\textless{}{-}} \FunctionTok{read.csv}\NormalTok{(}\StringTok{"data/cashless.csv"}\NormalTok{)}
\NormalTok{cashless }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{count}\NormalTok{(Transaction\_Habits)  }\CommentTok{\# Count number in each Stance category}
\end{Highlighting}
\end{Shaded}
...
In this study, the parameter of interest is the \textbf{population proportion of U.S. adults who report using only cashless purchases.}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  What assumption are we making about the value for this parameter?
\end{enumerate}
...
\item
  ...
  \vspace{0.3in}
\item
  Calculate the summary statistic (sample proportion) for this study. Use proper notation.
\end{enumerate}
...
\caption{Simulated Null Distribution with Normal Distribution Overlay}\label{fig:unnamed-chunk-2}
\end{figure}
\newpage
...
\emph{The standard error of the sample proportion measures the variability of possible sample proportions from the actual proportion. In other words, how far each possible sample proportion is from the actual proportion on average.}
...
\tightlist
\item
  Using the null standard error of the sample proportion, calculate the standardized sample proportion (Z).
\end{enumerate}
...
\caption{Standard Normal Curve}\label{fig:Ncurve}
\end{figure}

\emph{The value of 11.480 measures how far the sample proportion (0.410) is from the null value of 0.34. In other words, the sample proportion of U.S. adults that reported using only cashless purchases of 0.41 is 11.48 standard errors above the null value of 0.34.}

We will use the \texttt{pnorm()} function in \texttt{R} to find the p-value. The value for Z was entered into the code below to get the p-value. Notice that we used \texttt{lower.tail\ =\ FALSE} to find the p-value. \texttt{R} will calculate the p-value \emph{greater} than the value of the standardized statistic.
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{pnorm}\NormalTok{(}\FloatTok{11.480}\NormalTok{, }\CommentTok{\# Enter value of standardized statistic}
...
\textbf{The margin of error (ME)} is the value of the \(z^*\) multiplier times the standard error of the statistic.
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{qnorm}\NormalTok{(}\FloatTok{0.975}\NormalTok{, }\AttributeTok{lower.tail =} \ConstantTok{TRUE}\NormalTok{) }\CommentTok{\# Multiplier for 95\% confidence interval}
\end{Highlighting}
\end{Shaded}
...
\section{Activity 8: What does confidence mean?}\label{activity-8-what-does-confidence-mean}
...
\section{Activity 9: Errors and Power}\label{activity-9-errors-and-power}
...
\item
  ...
  \vspace{0.5in}
\item
  Use proper notation to write the null and alternative hypothesis the company would need to test in order to check their advertisement claim.
  \vspace{0.5in}
\end{enumerate}
...
\item
  ...
  \vspace{0.5in}
\item
  How does increasing the distance between the null and believed true probability of success affect the power of the test?
  \vspace{0.5in}
\item
...
\section{Module 6 Lab: Mixed Breed Dogs in the U.S.}\label{module-6-lab-mixed-breed-dogs-in-the-u.s.}
...
\tightlist
\item
  \textbf{Can theory-based methods be used to analyze these data? Explain your answer.}
\end{enumerate}
...
The \texttt{qnorm()} function in R will tell us the \(z^*\) value for the desired percentile (in this case, 90\% + 5\% = 95\% percentile).
...
\tightlist
\item
  What type of error may have occurred?
\end{enumerate}
...
\tightlist
\item
  Describe this error in the context of the study?
\end{enumerate}
...
\section{Guided Activity 9: The Good Samaritan}\label{guided-activity-9-the-good-samaritan}
...
\section{Activity 10: Poisonous Mushrooms}\label{activity-10-poisonous-mushrooms}
...
\begin{itemize}
\item
  Upload and open the R script file for the Module 7 Activity. Upload the csv file, \texttt{mushrooms\_edibility}.
\item
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{mushrooms }\OtherTok{\textless{}{-}} \FunctionTok{read.csv}\NormalTok{(}\StringTok{"datasetname"}\NormalTok{) }\CommentTok{\# Read data set in}
...
\section{Reading Guide: Theory-based Inference for a Difference in Proportions}\label{reading-guide-theory-based-inference-for-a-difference-in-proportions}
...
\section{Guided Activity 10: Winter Sports Helmet Use and Head Injuries --- Theory-based Methods}\label{guided-activity-10-winter-sports-helmet-use-and-head-injuries-theory-based-methods}
...
\tightlist
\item
  Calculate the standard error for a difference in proportions to find a 90\% confidence interval.
\end{enumerate}
...
\tightlist
\item
  Using the multiplier of \(z^*\) = 1.645 and the calculated standard error, calculate the \textbf{margin of error} for a 90\% confidence interval.
\end{enumerate}
...
\tightlist
\item
  Interpret the confidence interval found in question 4 in context of the problem.
\end{enumerate}
...
\subsubsection*{Use statistical analysis methods to draw inferences from the data}\label{use-statistical-analysis-methods-to-draw-inferences-from-the-data-4}
\addcontentsline{toc}{subsubsection}{Use statistical analysis methods to draw inferences from the data}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{pnorm}\NormalTok{(}\SpecialCharTok{{-}}\FloatTok{2.878}\NormalTok{, }\CommentTok{\# Enter value of standardized statistic}
      \AttributeTok{m=}\DecValTok{0}\NormalTok{, }\AttributeTok{s=}\DecValTok{1}\NormalTok{, }\CommentTok{\# Using the standard normal mean = 0, sd = 1}
...
\section{Activity 11: Diabetes}\label{activity-11-diabetes}
...
\begin{itemize}
\item
  Upload and open the R script file for this activity. Upload the csv file, \texttt{diabetes}.
\item
  Enter the name of the data set \texttt{datasetname} in the R script file in line 7.
\item
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{glycemic }\OtherTok{\textless{}{-}} \FunctionTok{read.csv}\NormalTok{(}\StringTok{"datasetname"}\NormalTok{)}
...
\item
  ...
  \vspace{0.3in}
\item
  Calculate the summary statistic (difference in proportions). Use appropriate notation.
  \vspace{0.3in}
\item
  Calculate the null standard error for the difference in sample proportions.
\end{enumerate}
...
\tightlist
\item
  How many standard errors is the sample difference in proportions from the null value?
\end{enumerate}

\vspace{0.6in}

Use the \texttt{pnorm()} function in R to find the p-value.
...
\begin{Shaded}
\begin{Highlighting}[]
\DecValTok{2}\SpecialCharTok{*}\FunctionTok{pnorm}\NormalTok{(xx, }\CommentTok{\# Enter value of standardized statistic}
      \AttributeTok{m=}\DecValTok{0}\NormalTok{, }\AttributeTok{s=}\DecValTok{1}\NormalTok{, }\CommentTok{\# Using the standard normal mean = 0, sd = 1}
      \AttributeTok{lower.tail=}\ConstantTok{FALSE}\NormalTok{) }\CommentTok{\# Gives a p{-}value less than the standardized statistic}
\end{Highlighting}
\end{Shaded}
...
\item
  ...
  \vspace{0.3in}
\item
  Will a 95\% simulation confidence interval contain the null value of zero? Explain your answer.
  \vspace{0.7in}
\item
  Calculate the standard error for a difference in proportions to create a 95\% confidence interval.\\
  \vspace{0.8in}
\item
  Use the multiplier of \(z^*\) = 1.96 and the standard error found in question 10 to calculate a 95\% confidence interval for the parameter of interest.
  \vspace{1in}
\item
...
\tightlist
\item
  Calculate the relative risk of glycemic control for patients given Rosiglitazone compared to patients given the placebo.
\end{enumerate}
...
\tightlist
\item
  Calculate the increase or decrease in risk of glycemic control for patients given Rosiglitazone compared to patients given the placebo.
\end{enumerate}
...
\tightlist
\item
  Interpret the increase or decrease in risk of glycemic control in context of the problem.
\end{enumerate}
...
\tightlist
\item
  Calculate the percent increase or percent decrease in glycemic control.
\end{enumerate}
...
\tightlist
\item
  Interpret the value of relative risk as a percent increase or percent decrease in glycemic control.
\end{enumerate}
...
\section{Module 8 Lab: Is yawning contagious?}\label{module-8-lab-is-yawning-contagious}
...
\subsection{Is yawning contagious?}\label{is-yawning-contagious}
...
\item
  Upload the csv file, \texttt{YawnData.csv}. Enter the name of the data set for \texttt{datasetname} in the R script file in line 7.
\item
  Highlight and run lines 1--8 to get the counts for each combination of categories.
\end{itemize}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{yawn }\OtherTok{\textless{}{-}} \FunctionTok{read.csv}\NormalTok{(}\StringTok{"datasetname"}\NormalTok{)}
\NormalTok{yawn }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{group\_by}\NormalTok{(YawnSeed) }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{count}\NormalTok{(Outcome)}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\FunctionTok{two\_proportion\_test}\NormalTok{(}\AttributeTok{formula =}\NormalTok{ response}\SpecialCharTok{\textasciitilde{}}\NormalTok{explanatory, }\CommentTok{\# response \textasciitilde{} explanatory}
         \AttributeTok{data=}\NormalTok{ yawn, }\CommentTok{\# Name of data set}
         \AttributeTok{first\_in\_subtraction =} \StringTok{"xx"}\NormalTok{, }\CommentTok{\# Order of subtraction: enter the name of Group 1}
         \AttributeTok{number\_repetitions =} \DecValTok{1000}\NormalTok{, }\CommentTok{\# Always use a minimum of 1000 repetitions}
         \AttributeTok{response\_value\_numerator =} \StringTok{"xx"}\NormalTok{, }\CommentTok{\# Define which outcome is a success }
         \AttributeTok{as\_extreme\_as =}\NormalTok{ xx, }\CommentTok{\# Calculated observed statistic (difference in sample proportions)}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{pnorm}\NormalTok{(xx, }\AttributeTok{lower.tail =} \ConstantTok{FALSE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\tightlist
\item
  Explain why the theory-based p-value does not match the p-value found using simulation methods.
\end{enumerate}
...
\tightlist
\item
  Fill in the missing values/names in the R script file in the two-proportion\_bootstrap\_CI function to create the null distribution and find the 90\% simulation confidence interval.
\end{itemize}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{two\_proportion\_bootstrap\_CI}\NormalTok{(}\AttributeTok{formula =}\NormalTok{ response}\SpecialCharTok{\textasciitilde{}}\NormalTok{explantory, }\CommentTok{\# response\textasciitilde{}explanatory,}
                            \AttributeTok{data =}\NormalTok{ yawn, }\CommentTok{\# Name of data set}
                            \AttributeTok{first\_in\_subtraction =} \StringTok{"xx"}\NormalTok{, }\CommentTok{\# Order of subtraction; enter the name of Group 1}
                            \AttributeTok{number\_repetitions =} \DecValTok{1000}\NormalTok{, }\CommentTok{\#Always use a minimum of 1000 repetitions}
                            \AttributeTok{response\_value\_numerator =} \StringTok{"xx"}\NormalTok{, }\CommentTok{\# Define which outcome is a success}
...
\chapter{Inference for Inference for a Single Mean or Paired Mean Difference}\label{inference-for-inference-for-a-single-mean-or-paired-mean-difference}
...
\section{Guided Activity 11: Color Interference}\label{guided-activity-11-color-interference}
...
\section{Guided Activity 12: Snakes}\label{guided-activity-12-snakes}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{snakes }\OtherTok{\textless{}{-}} \FunctionTok{read.csv}\NormalTok{(}\StringTok{"data/vipers.csv"}\NormalTok{)}
...
\NormalTok{snakes\_diff }\OtherTok{\textless{}{-}}\NormalTok{ snakes }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{differences =}\NormalTok{ DistanceMaze }\SpecialCharTok{{-}}\NormalTok{ DistanceOpen)}
...
\begin{Highlighting}[]
    \FunctionTok{paired\_test}\NormalTok{(}\AttributeTok{data =}\NormalTok{ snakes\_diff}\SpecialCharTok{$}\NormalTok{differences,   }\CommentTok{\# Vector of differences }
                                         \CommentTok{\# or data set with column for each group}
            \AttributeTok{shift =} \FloatTok{14.454}\NormalTok{,   }\CommentTok{\# Shift needed for bootstrap hypothesis test}
            \AttributeTok{as\_extreme\_as =} \SpecialCharTok{{-}}\FloatTok{14.454}\NormalTok{,  }\CommentTok{\# Observed statistic}
...
\section{Activity 12: Swearing}\label{activity-12-swearing}
...
\item
  Upload the csv file, \texttt{pain\_tolerance}.
\item
  Enter the name of the data set for datasetname in the R script file in line 8.
\item
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{swearing }\OtherTok{\textless{}{-}} \FunctionTok{read.csv}\NormalTok{(}\StringTok{"datasetname"}\NormalTok{)}
...
\tightlist
\item
  Simulate a null distribution and compute the p-value. Using the R script file for this lab, enter your answers for question 9 in place of the \texttt{xx}'s to produce the null distribution with 1000 simulations. Highlight and run lines 30--36.
\end{enumerate}
...
\tightlist
\item
  Using bootstrapping and the provided R script file, find a 98\% confidence interval. Fill in the missing values/numbers in the \texttt{paired\_bootstrap\_CI()} function to create the 98\% confidence interval. Highlight and run lines 41--44.
\end{enumerate}
...
\tightlist
\item
  Interpret the \emph{confidence level} of the interval found in question 13.
\end{enumerate}
...
\section{Guided Activity 13: Does behavior impact performance?}\label{guided-activity-13-does-behavior-impact-performance}
...
\def\labelenumi{\arabic{enumi}.}
\item
  This activity differs from the activities in Module 9 because the responses are independent, not paired. These data are analyzed as a difference in means, not a mean difference.
\item
...
\section{Guided Activity 14: Moon Phases and Virtual Reality}\label{guided-activity-14-moon-phases-and-virtual-reality}
...
When we are comparing two quantitative variables to find the degrees of freedom to use for the t-distribution, we need to use the group with the smallest sample size and subtract 1. (\texttt{df} = minimum of \(n_1 - 1\) or \(n_2 - 1\)).

\vspace{0.2in}

\begin{Shaded}
\begin{Highlighting}[]
\DecValTok{2}\SpecialCharTok{*}\FunctionTok{pt}\NormalTok{(}\FloatTok{0.613}\NormalTok{, }\AttributeTok{df=}\DecValTok{55}\NormalTok{, }\AttributeTok{lower.tail=}\ConstantTok{FALSE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\section{Activity 13: Dinosaurs}\label{activity-13-dinosaurs}
...
\item
  Upload the csv file, \texttt{dinosaur}.
\item
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{dinos }\OtherTok{\textless{}{-}} \FunctionTok{read.csv}\NormalTok{(}\StringTok{"datasetname"}\NormalTok{)}
...
\item
  ...
  \vspace{0.4in}
\item
  How far, on average, is each difference in mean femur length from the difference in true mean femur length? What is the appropriate notation for this value?
\end{enumerate}
...
\subsection*{Hypothesis test}\label{hypothesis-test-5}
\addcontentsline{toc}{subsection}{Hypothesis test}
...
\subsection*{Confidence interval}\label{confidence-interval-8}
\addcontentsline{toc}{subsection}{Confidence interval}
...
\section{Module 10 Lab: Trustworthiness}\label{module-10-lab-trustworthiness}
...
\section{Video Notes: Inference for Two Quantitative Variables}\label{video-notes-inference-for-two-quantitative-variables}
...
\section{Guided Activity 15: Prediction of Crocodilian Body Size}\label{guided-activity-15-prediction-of-crocodilian-body-size}
...
\tightlist
\item
  Suppose we wanted to complete the simulation test using correlation as the summary measure, instead of slope. Which two inputs in the regression\_test function would need to be changed to test for correlation? What inputs should you use instead?
\end{enumerate}
...
\tightlist
\item
  Interpret the confidence interval in context of the problem. \emph{Hint: use the interpretation of slope in your confidence interval interpretation.}
\end{enumerate}
...
\section{Guided Activity 16: Golf Driving Distance}\label{guided-activity-16-golf-driving-distance}
...
\paragraph*{Confidence interval}\label{confidence-interval-9}
\addcontentsline{toc}{paragraph}{Confidence interval}
...
\section{Module 11 Lab: Big Mac Index}\label{module-11-lab-big-mac-index}
...
Using the R script file for this activity, enter your answers for question 9 in place of the \texttt{xx}'s to produce the null distribution with 1000 simulations.
...
\section{Reading Guide: Special Topics}\label{reading-guide-special-topics}
...
\section{Guided Activity 17: Probability Studies}\label{guided-activity-17-probability-studies}

\setstretch{1}

\subsection{Learning outcomes}\label{learning-outcomes-35}
...
\item
  ...
  \vspace{.8in}
\item
  Find the probability that a deer that is positive for CWD did not survive. What is the notation used for this probability?
  \vspace{.8in}
\item
...
\subsection{Learning outcomes}\label{learning-outcomes-36}
...
\subsection{Additional notes}\label{additional-notes-29}
...
\tightlist
\item
  Use the appropriate Final Exam Review R script file to analyze the following research question, ``Is there evidence that adult Islanders that drink a caffeinated drink have an increase in blood glucose level, on average?'' Use before \(-\) after as the order of subtraction.
\end{enumerate}
...
\tightlist
\item
  Use the appropriate Final Exam Review R script file to analyze the following research question: ``Is there evidence that the height of adult Islanders is associated with the their balance time?''
\end{enumerate}
