...
Example: The Bureau of Transportation Statistics (\emph{Bureau of Transportation Statistics} 2019) collects data on all forms of public transportation. The data set seen here includes several variables collect on flights departing on a random sample of 150 US airports in December of 2019.
...
\begin{itemize}
\item
  \textbf{Conditions for the sampling distribution of \(\bar{y}_1 - \bar{y}_2\) to follow an approximate normal distribution}:
...
\section{Activity 18: Does behavior impact performance?}\label{activity-18-does-behavior-impact-performance}
...
\FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ condition, }\AttributeTok{y =}\NormalTok{ number\_of\_uses)) }\SpecialCharTok{+}
    \FunctionTok{geom\_boxplot}\NormalTok{() }\SpecialCharTok{+} 
    \FunctionTok{labs}\NormalTok{(}\AttributeTok{title =} \StringTok{"Side by side boxplots of Number of Uses for a Brick }
\StringTok{    based on Behavior Condition for College Students in a }
\StringTok{    Management Course"}\NormalTok{,}
         \AttributeTok{x =} \StringTok{"Behavior Condition"}\NormalTok{,}
...
Using the R script file for this activity, enter your answers for question 5 in place of the \texttt{xx}'s to produce the bootstrap distribution with 10000 simulations to create a 99\% confidence interval; highlight and run lines 35--39.
...
\section{Activity 19: Moon Phases and Virtual Reality}\label{activity-19-moon-phases-and-virtual-reality}
...
\begin{itemize}
\item
  \textbf{Independence}: the sample's observations are independent and there is independence between groups. (\emph{Remember}: This also must be true to use simulation methods!)
\item
  \textbf{Normality Condition}: either the sample observations come from a normally distributed population or we have a large enough sample size. \emph{When we have two samples, we need to check this condition for each group!} To check this condition, use the following rules of thumb (for both \(n_1\) and \(n_2\)):
...
\addcontentsline{toc}{subsubsection}{Plot of the data}
...
\NormalTok{dinos }\SpecialCharTok{\%\textgreater{}\%}
  ...
  \FunctionTok{geom\_boxplot}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{title =} \StringTok{"Don\textquotesingle{}t forget to title your plot!"}\NormalTok{, }\CommentTok{\#Title should include type of plot, }
       \CommentTok{\#observational units, and variables plotted}
       \AttributeTok{x =} \StringTok{"Label the x{-}axis"}\NormalTok{, }\CommentTok{\#add an x{-}axis label}
       \AttributeTok{y =} \StringTok{"Label the y{-}axis (Don\textquotesingle{}t forget units!)"}\NormalTok{) }\CommentTok{\#add a y{-}axis label}
\end{Highlighting}
\end{Shaded}
...
Module 12 will cover exploratory data analysis and both simulation-based and theory-based methods of inference for a quantitative response variable with paired samples. The \textbf{summary measure} for paired data is a \textbf{mean difference}.
...
\begin{itemize}
\item
  \textbf{Conditions for the sampling distribution of \(\bar{y}_d\) to follow an approximate normal distribution}:
...
Example: Is there a difference in heights between husbands and wives? The heights were measured on the husband and wife in a random sample of 199 married couples from Great Britain (\emph{Great Britain Married Couples}, n.d.).
...
\section{Activity 20: Paired vs.~Independent Samples}\label{activity-20-paired-vs.-independent-samples}
...
\subsubsection*{R Instructions}\label{r-instructions-14}
\addcontentsline{toc}{subsubsection}{R Instructions}
...
\NormalTok{tat\_diff }\SpecialCharTok{\%\textgreater{}\%} 
    ...
    \FunctionTok{geom\_boxplot}\NormalTok{()}\SpecialCharTok{+}
    \FunctionTok{labs}\NormalTok{(}\AttributeTok{title=}\StringTok{"Boxplot of the difference in Sweat Rate (mg per (cm\^{}2*min))}
\StringTok{         for Adult Men with Tattoos comparing }
...
\section{Activity 21: Color Interference}\label{activity-21-color-interference}
...
Like with a single mean, for paired data we will need to \emph{shift} each data point by the difference \(\mu_0 - \bar{y}_d\).
...
\setcounter{enumi}{3}
\item
  What is the value of \(\bar{y}_d\)? What is the sample size?
  \vspace{0.25in}
\item
...
\subsection*{Use statistical inferential methods to draw inferences from the data}\label{use-statistical-inferential-methods-to-draw-inferences-from-the-data-6}
\addcontentsline{toc}{subsection}{Use statistical inferential methods to draw inferences from the data}
...
\section{Activity 22: IPEDS (continued) --- Linear Regression}\label{activity-22-ipeds-continued-linear-regression}
...
\tightlist
\item
  The linear model and correlation output for the data set with the added point is provided below. Did the slope and/or value of correlation change in a noticeable way?
\end{enumerate}
...
\tightlist
\item
  Earlier in the semester, we learned that the median and the IQR are resistant to outliers. In other words, they are not influenced by the presence of outliers. Are the values of slope and correlation resistant to outliers? Explain your answer using the previous examples in this activity.
\end{enumerate}
...
\tightlist
\item
  Use the correlation, \(r\), found in the correlation matrix, to calculate the coefficient of determination between bill depth and body mass, \(r^2\), for Antarctica penguins.
\end{enumerate}
...
\tightlist
\item
  The coefficient of determination between bill depth and flipper length is 33.5\%. Calculate the correlation between these variables from this value. How can we tell if the value of correlation should be positive or negative?
\end{enumerate}
...
\subsubsection*{Coefficient of determination (revisited)}\label{coefficient-of-determination-revisited}
\addcontentsline{toc}{subsubsection}{Coefficient of determination (revisited)}
...
\section{Activity 24: Golf Driving Distance}\label{activity-24-golf-driving-distance}
...
\subsection*{R Instructions}\label{r-instructions-17}
\addcontentsline{toc}{subsection}{R Instructions}
...
\tightlist
\item
  Write out the null and alternative hypotheses in notation to test \emph{slope} between Big Mac cost and log GDP.
\end{enumerate}
...
Use the \texttt{regression\_test()} function in R (in the \texttt{catstats} package) to simulate the null distribution of sample \textbf{slopes} and compute a p-value. We will need to enter the response variable name and the explanatory variable name for the formula, the data set name (identified above as \texttt{mac}), the summary measure used for the test, number of repetitions, the sample statistic (value of correlation), and the direction of the alternative hypothesis.
...
We will use the \texttt{regression\_bootstrap\_CI()} function in R (in the \texttt{catstats} package) to simulate the bootstrap distribution of sample \textbf{slopes} and calculate a 90\% confidence interval.
...
\section{Module 12 Review - Paired Data}\label{module-12-review---paired-data}
...
\tightlist
\item
  Calculate the difference \(\mu_0 - \bar{y}_d\). Will we need to shift the data up or down?
  \vspace{0.5in}
\end{enumerate}
