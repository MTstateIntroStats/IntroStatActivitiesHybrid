...
Later in the semester we will learn more about how to use \texttt{R} - for today we will have a short introduction using the \texttt{R} studio server. Download the provided \texttt{R} script file from canvas.
...
\tightlist
\item
  Login to the \texttt{R} studio server using your netID login. Upload the file to the server using the following instructions.
\end{enumerate}
...
\item
  Once the file is uploaded, you should see the uploaded file appear in the list of files in the bottom right window. Click on the \texttt{R} script file name to open the file in the Editor window (upper left window).
\end{itemize}
...
\tightlist
\item
  In the \texttt{R} script file, highlight and run lines 1--7
\end{itemize}
...
The following is a plot of 1000 random samples of size 10 created in \texttt{R}. To re-create this plot run lines 11--27 in the \texttt{R} script file.

\begin{center}\includegraphics[width=0.65\linewidth]{01-A02-samplingbias_files/figure-latex/unnamed-chunk-3-1} \end{center}

It would be useful to know what the center (mean) is for this sampling distribution. The distribution you created in the \texttt{R} script file may vary a little from what is reported in your coursepack. To find the mean and standard deviation from your distribution run lines 31--32.
...
Using the R script file for this activity, enter your answers for question 5 in place of the \texttt{xx}'s to produce the bootstrap distribution with 10000 simulations; highlight and run lines 33--37.
...
\item
  Highlight and run lines 34--38. \textbf{Upload a copy of the bootstrap distribution created to Gradescope for your group.}
\end{itemize}
...
The linear model function in R (\texttt{lm()}) gives us the summary for the least squares regression line. The estimate for \texttt{(Intercept)} is the \(y\)-intercept for the line of least squares, and the estimate for \texttt{STAMath\_75} (the \(x\)-variable name) is the value of \(b_1\), the slope.
...
\tightlist
\item
  Note that the slope of the line has not changed much between the original data set and the data set with the values removed. Would you consider either of these points to be influential?
\end{enumerate}
...
\tightlist
\item
  In the next plot, note that three extreme values have been added to the data set. Circle these points on the plot.
\end{enumerate}
...
\tightlist
\item
  The linear model and correlation output for the data set with the added points is provided below. Did the slope and/or value of correlation change?
\end{enumerate}
...
\tightlist
\item
  Upload and import the \texttt{Antarctica\_Penguins} csv file and the provided R script file for this activity. Enter the name of the data set for \texttt{datasetname} in the R script file in line 4\ldots.
\end{itemize}
...
\NormalTok{penguins }\SpecialCharTok{\%\textgreater{}\%}
  ...
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{x =} \StringTok{"flipper length (mm)"}\NormalTok{,  }\CommentTok{\# Label x{-}axis}
       ...
       \AttributeTok{title =} \StringTok{"Scatterplot of Body Mass by Flipper Length for }
\StringTok{       Antarctica Penguins"}\NormalTok{) }\SpecialCharTok{+} \CommentTok{\# Be sure to title your plots}
  \FunctionTok{geom\_smooth}\NormalTok{(}\AttributeTok{method =} \StringTok{"lm"}\NormalTok{, }\AttributeTok{se =} \ConstantTok{FALSE}\NormalTok{)  }\CommentTok{\# Add regression line}
\end{Highlighting}
\end{Shaded}
...
\begin{itemize}
\item
  Enter \texttt{body\_mass\_g} for response and \texttt{flipper\_length\_mm} for explanatory in line \ldots{}
\item
...
\tightlist
\item
  Predict the body mass of a penguin with a flipper length of 230mm.
\end{enumerate}
...
\tightlist
\item
  Calculate the residual for a penguin with a flipper length of 230mm and a body mass of 6050g.
\end{enumerate}
...
\tightlist
\item
  What is the value of correlation between flipper length and body mass?
\end{enumerate}
...
\tightlist
\item
  Use the correlation, \(r\), found in question 6, to calculate the coefficient of determination between flipper length and body mass, \(r^2\).
\end{enumerate}
...
\textbf{The calculated value for the SST is 215704477.919.}
...
\textbf{The calculated value for the SSE is 51230376.045.}
...
\tightlist
\item
  Describe the relationship between bill depth and body mass.
\end{enumerate}
...
\NormalTok{penguins }\SpecialCharTok{\%\textgreater{}\%} \CommentTok{\# Data set pipes into...}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ bill\_depth\_mm, }\AttributeTok{y =}\NormalTok{ body\_mass\_g, }\AttributeTok{shape =}\NormalTok{ species, }\AttributeTok{color=}\NormalTok{species))}\SpecialCharTok{+}  \CommentTok{\# Specify variables}
    \FunctionTok{geom\_point}\NormalTok{(}\AttributeTok{alpha=}\FloatTok{0.5}\NormalTok{) }\SpecialCharTok{+}  \CommentTok{\# Add scatterplot of points}
...
\textbf{Note that the relationship between bill depth and body mass is now positive based on the penguin species. This is an example of what?}
...
\textbf{Is species of Antarctica penguins associated with body mass? Is species of Antarctica penguins associated with bill depth?}
...
\NormalTok{mac }\SpecialCharTok{\%\textgreater{}\%} \CommentTok{\# Pipe data set into...}
  ...
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{x =} \StringTok{"include an x{-}axis label (don\textquotesingle{}t forget units)"}\NormalTok{,  }\CommentTok{\# Label x{-}axis}
       ...
       \AttributeTok{title =} \StringTok{"A title should include type of plot, OUs, variables"}\NormalTok{) }\SpecialCharTok{+}  \CommentTok{\# Be sure to tile your plots}
  \FunctionTok{geom\_smooth}\NormalTok{(}\AttributeTok{method =} \StringTok{"lm"}\NormalTok{, }\AttributeTok{se =} \ConstantTok{FALSE}\NormalTok{)  }\CommentTok{\# Add regression line}
\end{Highlighting}
\end{Shaded}
