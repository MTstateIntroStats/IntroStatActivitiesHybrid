## Activity 10: Summarizing Quantitative Variables
...
The `favstats()` function from the `mosaic` package gives the summary statistics for a quantitative variable. The R output below provides the summary statistics for the variable `Cost_InState`. The summary statistics provided are the two measures of center (mean and median) and two measures of spread (standard deviation and the quartile values to calculate the IQR) for undergraduate In-State tuition cost. 
...
IPEDS %>%
    summarize(favstats(Cost_InState))
```

```
#>    min      Q1 median      Q3   max     mean       sd    n missing
#> 1 5075 24110.5  35275 50485.5 78555 38398.06 16484.52 1679     288
```
...
There are three type of plots used to plot a single quantitative variable: a dotplot, a histogram or a boxplot.  A dotplot of In-state tuition costs would plot a dot for the In-State tuition cost for each 4-year US higher education institution.

First, let's create a histogram of the variable `Cost_InState`.

* Enter the name of the variable, `Cost_InState` in line 19 for `variable` in the R script file.  
...
ggplot(aes(x = Cost_InState)) +   # Name variable to plot
  geom_histogram(binwidth = 10000, boundary = 0, closed = "left") +  
    # Create histogram with specified binwidth
  labs(title = "Don't forget to title the plot!", # Title for plot
       x = "In-State Tuition Cost ($)", # Label for x axis
       y = "Frequency") # Label for y axis
```



\begin{center}\includegraphics[width=0.6\linewidth]{06-A10-EDA-quantitative_files/figure-latex/unnamed-chunk-2-1} \end{center}

Notice that the **bin width** for the histogram is 10000.  For example the first bin consists of the number of institutions in the data set with an In-State cost of tuition of \$0 to \$10,000.  It is important to note that a tuition cost on the boundary of a bin will fall into the bin above it; for example, 10000 would be counted in the bin 10000--20000. 

**Which range of In-State Tuition Costs have the highest frequency?**

\vspace{0.4in}

Next we will create a boxplot of the variable `Cost_InState`.  
...
ggplot(aes(x = Cost_InState)) +   # Name variable to plot
  ...
  labs(title = "Boxplot of In-State Tuition Cost for 4-year \n Higher Education US Institutions", 
           # Title for plot
           # Note the \n starts a new line
       x = "In-State Tuition Cost ($)", # Label for x axis
...
2.  What is the shape of the distribution of In-State tuition cost?
...
\begin{center}\includegraphics[width=0.7\linewidth]{06-A10-EDA-quantitative_files/figure-latex/unnamed-chunk-4-1} \end{center}
...
\begin{center}\includegraphics[width=0.7\linewidth]{06-A10-EDA-quantitative_files/figure-latex/unnamed-chunk-5-1} \end{center}
