...
* Enter the name of the data set (see the environment tab) for datasetname.csv in the R script file in line 7. 

* Highlight and run lines 1--7 to load the dataset.
...
* Enter the name of the explanatory variable and the name of the response variable in the code

* Highlight and run line 11 to get the counts for each combination of categories


``` r
mushrooms %>% group_by(explanatory) %>% count(response) #finds the counts in each group
```
...
* Make sure to title your plot and label the axes  


``` r
mushrooms %>% # Data set piped into...
  ggplot(aes(x = explanatory, fill = response)) +   # This specifies the variables
  geom_bar(stat = "count", position = "fill") +  # Tell it to make a stacked bar plot
  labs(title = "Don't forget a title!",  # Make sure to title your plot
       x = "Add x-axis label",   # Label the x axis
       y = "", # Remove y axis label
       fill = "Add fill label") +  # Fill label
...
11.  Write a paragraph summarizing the results of the study.  **Upload your group's paragraph to Gradescope.** Be sure to describe:
