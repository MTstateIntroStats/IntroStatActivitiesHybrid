## Activity 15:  Summarizing Two Categorical Variables
...
ggplot(aes(x = Light, fill = Sight)) +   # This specifies the variables
  ...
  labs(title = "Segmented Bar Plot of Night Light Use by Level of
       ...
       fill = "Level of Myopia")  +  # Fill label
  scale_fill_grey()  # Make figure black and weight
```



\begin{center}\includegraphics[width=0.75\linewidth]{08-A15-EDA-categorical_files/figure-latex/unnamed-chunk-3-1} \end{center}
...
myopia %>% # Data set piped into...
  ...
  geom_mosaic(aes(x=product(Sight, Light), fill = Sight)) +  # Tell it to make a mosaic plot
...
\begin{center}\includegraphics[width=0.75\linewidth]{08-A15-EDA-categorical_files/figure-latex/unnamed-chunk-4-1} \end{center}
...
Activities 14--16 cover the material in Module 8.  For more practice with exploratory data analysis (difference in proportion and relative risk) use the Module 8 Review worksheet in the Unit 2 Review Materials (pgs. 255--256).
