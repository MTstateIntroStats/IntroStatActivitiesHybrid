...
\begin{center}\includegraphics[width=0.47\linewidth]{13-L13-regression-wrapup_files/figure-latex/unnamed-chunk-4-1} \includegraphics[width=0.47\linewidth]{13-L13-regression-wrapup_files/figure-latex/unnamed-chunk-4-2} \end{center}
...
5. Write out the null and alternative hypotheses in notation to test *slope* between Big Mac cost and log GDP.
...
9. Give the value of the slope of the regression line.  Interpret this value in context of the problem.
\vspace{0.6in}
...
Use the `regression_test()` function in R (in the `catstats` package) to simulate the null distribution of sample **slopes** and compute a p-value.  We will need to enter the response variable name and the explanatory variable name for the formula, the data set name (identified above as `mac`), the summary measure used for the test, number of repetitions, the sample statistic (value of correlation), and the direction of the alternative hypothesis.

The response variable name is `log_GDP` and the explanatory variable name is `dollar_price`.

Using the R script file for this activity, produce the null distribution with 10000 simulations. 
...
We will use the `regression_bootstrap_CI()` function in R (in the `catstats` package) to simulate the bootstrap distribution of sample **slopes** and calculate a 90\% confidence interval. 
...
14. Write a paragraph summarizing the results of the study as if you are reporting these results in your local newspaper.  **Upload a copy of your paragraph to Gradescope for your group.**  Be sure to describe:
